...
public class Pessoa {
    ...
    public Pessoa(String nome, String sobrenome, int dia, int mes, int ano) {
        ...
        if (ValidaData.isDia(dia) && ValidaData.isMes(String.valueOf(mes)) && ValidaData.isAno(ano) && ValidaData.isDataValida(dia, mes, ano)) {
            dataNasc = new GregorianCalendar(ano, mes, dia);
        } else {
            throw new IllegalArgumentException("Data inválida. Insira uma data existente.");
        }
        CONTADOR += 1;
    }
